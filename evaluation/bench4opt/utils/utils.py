...
def graph_generator(ins_name):
    ...
    for i in range(ncons):
        ...
        for j in range(nvars):
            ...
            coeff = m.getCoeff(
                cons, var
            )  # Gurobi-specific function for constraint coefficients
...
def concate_color(color_cons, color_var):
    ...
    color = torch.zeros(
        len(color_cons) + len(color_var), len(color_cons[0]) + len(color_var[0])
    )
    ...
    for j in range(len(color_cons), len(color_cons) + len(color_var)):
        color[j] = F.pad(
            color_var[j - len(color_cons)], (len(color_cons[0]), 0), "constant", 0
        )
    return color
...
def derive_adjacency(A1):
    ...
    Adj = torch.cat(
        (torch.cat((zeros_lower, A1), dim=1), torch.cat((A1.T, zeros_upper), dim=1)),
        dim=0,
    )
    return Adj
...
def get_cluster(
    same_color_partition, unique_set, partition_index, sub_graph_nodes, Adj
):
    ...
    while (
        sum(
            [
                ...
                if k in same_color_partition[partition_index]
            ]
        )
        - previous_sum
        > 0
        ...
        for i in same_color_partition[partition_index]:
            if i not in labeled_nodes:
                sub_graph_nodes[i] = sub_graph_nodes[i] + get_nbhd(
                    sub_graph_nodes[i], unique_set, Adj
                )
...
def derive_subgraphs(color, Adj):
    ...
    sub_graph_nodes = {}
    multiplicity_nodes = [
        item for item in list(range(len(Adj))) if item not in unique_set
    ]
    condition = True
    state = "uncheck"
    while condition:
        ...
        if sum([len(v) for v in sub_graph_nodes.values()]) < len(Adj) - len(unique_set):
            condition = True
            state = "continue"
        elif sum([len(v) for v in sub_graph_nodes.values()]) == len(Adj) - len(
            unique_set
        ):
            condition = False
            state = "complete"
...
def check_color_equivalence(color1, color2):
    ...
    for i in samecolor_cluster1.keys():
        if i not in samecolor_cluster2.keys():
            print(
                "-------------An unique color occurs in color1 but not in color2-----------"
            )
            return False
        else:
