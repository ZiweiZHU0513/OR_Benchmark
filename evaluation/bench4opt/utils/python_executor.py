...
with open('/home/liziniu/project/or_modeling/data/bench4opt/lp_data/data_LP_10_Network Flow Problem_[telecommunications_network]_[var5].json', 'r') as f:
    data = json.load(f)
...
for node in nodes:
    if node == source:
        model.addConstr(
            gp.quicksum(flow[i] for i, (start, end) in enumerate(edges) if start == node) -
            gp.quicksum(flow[i] for i, (start, end) in enumerate(edges) if end == node) == 1,
            "flow_conservation_source"
        )
    elif node == sink:
        model.addConstr(
            gp.quicksum(flow[i] for i, (start, end) in enumerate(edges) if end == node) -
            gp.quicksum(flow[i] for i, (start, end) in enumerate(edges) if start == node) == 1,
            "flow_conservation_sink"
        )
    else:
        model.addConstr(
            gp.quicksum(flow[i] for i, (start, end) in enumerate(edges) if end == node) ==
...
# Add energy efficiency constraint
model.addConstr(
    gp.quicksum(energies[i] * flow[i] for i in range(len(edges))) <= energy_threshold,
    "energy_efficiency"
)

# Optimize model
model.write('/tmp/tmpdgq1f8o4/model_test.lp')
# model.optimize()
#
#
# # Print the optimal flows
# if model.status == GRB.OPTIMAL:
# for i in range(len(edges)):
# print(f"Flow on edge {edges[i]}: {flow[i].x}")
# else:
